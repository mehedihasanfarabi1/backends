...
export default function BagTypeEdit() {
  const { id } = useParams();
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const res = await bagTypeApi.retrieve(id);
        setFormData({
          ...res,
          session: res.session || new Date().getFullYear(),
          is_default: Boolean(res.is_default),
          is_active: Boolean(res.is_active),
        });
      } catch (err) {
        console.error(err);
        Swal.fire("Error", "Failed to load Bag Type", "error");
      } finally {
        setLoading(false);
      }
    };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      await bagTypeApi.update(id, payload);
      Swal.fire("Success", "Bag Type Updated Successfully", "success");
      nav("/admin/bag-types");
    } catch (err) {
      console.error(err);
      Swal.fire("Error", "Failed to update Bag Type", "error");
    }
  };
    ...
    <div className="container mt-4 d-flex justify-content-center">
      <div className="card shadow-sm" style={{ width: "700px" }}>
        <div className="card-header bg-warning text-white text-center">
          <h5 className="mb-0">Edit Bag Type</h5>
        </div>

...
export default function BasicSettingEditForm() {
  const { id } = useParams();
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        const data = await basicSettingsApi.retrieve(id);
        ...
        console.error(err);
        toast.error("Failed to load basic setting data!");
      }
    };
    loadData();
  ...
  const handleSubmit = async (e) => {
    ...
    // Submit form
    try {
      await basicSettingsApi.update(id, {
        ...form,
        factory_id: parseInt(form.factory_id),
      });
      toast.success("Basic Setting updated!");
      Swal.fire("Success", "Basic Setting updated!", "success");
    ...
    <div className="container my-3">
      ...
      <div className="card shadow-sm">
        <div className="card-header bg-primary text-white py-2">
          <h6 className="mb-0">Edit Basic Setting</h6>
        </div>
